...
-- 2023-02-28,CTruong,Created File
-- 2023-03-01,CTruong,Completed File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_CTruong')
	 Begin 
	  Alter Database [Assignment07DB_CTruong] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_CTruong;
	 End
	Create Database Assignment07DB_CTruong;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_CTruong;
...
AS
SELECT TOP 100000 c.CategoryName, InventoryDate = DateName(mm, i.InventoryDate) + ',' + DateName(yy, i.InventoryDate), InventoryCountByCategory = SUM (i.Count)
FROM vCategories as c INNER JOIN vProducts as p
	ON c.CategoryID = p.CategoryID
		INNER JOIN vInventories as i
			ON p.ProductID = i.ProductID
GROUP BY c.CategoryName, i.InventoryDate
ORDER BY c.CategoryName, CAST (i.InventoryDate AS DATE),InventoryCountByCategory
GO
-- Check that it works: 
...
AS
SELECT TOP 100000 ProductName, InventoryDate, InventoryCount, PreviousMonthCount, CountVsPreviousCountKPI = Case
